...
public class ListController {
    ...
    public String addIpToList(@ModelAttribute("address") String ipAddress,
	    @ModelAttribute("listType") String listType, Map<String, Object> map) {
	try {
	    LOGGER.info("IP type: " + listType);
	...
	try {
	    ...
	    ipList = (List<IpAddress>) ipStatusListService.findIpList(
		    (pageNumber - 1) * countIpPerPage, countIpPerPage, ipType,
		    "whiteList");
	    ipCount = ipStatusListService.findIpCount(ipType, "whiteList")
		    .intValue();
	...
	try {
	    ...
	    ipList = (List<IpAddress>) ipStatusListService.findIpList(
		    (pageNumber - 1) * countIpPerPage, countIpPerPage, ipType,
		    "blackList");
	    ipCount = ipStatusListService.findIpCount(ipType, "blackList")
		    .intValue();
	    pageCount = ipCount / countIpPerPage + 1;
	    pageList = paginationService.loadPages();
	    map.put("ipList", ipList);
