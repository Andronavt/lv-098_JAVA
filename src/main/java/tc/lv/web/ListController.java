...
public class ListController {
    ...
    public String addIpToWhiteList(@ModelAttribute("address") String ipAddress,
        ...
        try {
            if (listType == 1) {
                if (IpValidator.isIpV4(ipAddress)) {
                    return addIpToDataBase(whiteListService.saveIpV4ByName(ipAddress), ipAddress, listType, map);
                }

                if (IpValidator.isIpV6(ipAddress)) {
                    return addIpToDataBase(whiteListService.saveIpV6ByName(ipAddress), ipAddress, listType, map);
                }
            } else {
                if (IpValidator.isIpV4(ipAddress)) {
                    return addIpToDataBase(blackListService.saveIpV4ByName(ipAddress), ipAddress, listType, map);

                }

                if (IpValidator.isIpV6(ipAddress)) {
                    return addIpToDataBase(blackListService.saveIpV6ByName(ipAddress), ipAddress, listType, map);
                }
            }
    ...
    // add IP to DB
    private String addIpToDataBase(boolean flag, String ipAddress, int listType, Map<String, Object> map)
            throws WhiteListServiceException {
