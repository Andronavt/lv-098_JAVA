...
public class GeoIpUtil {
	private static final String DIR = System.getenv("LV098_JAVA")
	...
	public void init() throws GeoIpException {
		try {
			lookupServiceIpV4Country = new LookupService(DIR
					+ GEO_IPV4_DB_COUNTRY, LookupService.GEOIP_MEMORY_CACHE);
			lookupServiceIpV6Country = new LookupService(DIR
					+ GEO_IPV6_DB_COUNTRY, LookupService.GEOIP_MEMORY_CACHE);
			lookupServiceIpV4City = new LookupService(DIR + GEO_IPV4_DB_CITY,
					LookupService.GEOIP_MEMORY_CACHE);
			lookupServiceIpV6City = new LookupService(DIR + GEO_IPV6_DB_CITY,
					LookupService.GEOIP_MEMORY_CACHE);
		} catch (Exception e) {
	...
	public City findLocationByIpAddress(String ipAddress) throws GeoIpException {
		...
		try {
			if (ipValidator.isIpV4(ipAddress)) {
				countryName = lookupServiceIpV4Country.getCountry(ipAddress)
						.getName();
				countryCode = lookupServiceIpV4Country.getCountry(ipAddress)
						.getCode();
				...
				countryName = lookupServiceIpV6Country.getCountry(ipAddress)
						.getName();
				countryCode = lookupServiceIpV6Country.getCountry(ipAddress)
						.getCode();
