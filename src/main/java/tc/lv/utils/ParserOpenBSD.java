...
public class ParserOpenBSD implements ParserInterface {
	...
	public ParserResults parse(File f) {
			...
			while (line.hasNext()) {
				...
				if (matcher.find()) {
					...
					if (IpValidator.isIpV4(ipStr)) {
						System.out.println(parserResults.ip4List.add(new IpV4Address(ipStr, new Date())));
					} else if (IpValidator.isIpV6(ipStr)) {
						parserResults.ip6List.add(new IpV6Address(ipStr, new Date()));
					} else {
						parserResults.notValidList.add(new NotValidIp(ipStr, new Date()));
					}
				}
			}
		...
		System.out.println(" LIST SIZE " + parserResults.ip4List.size());
