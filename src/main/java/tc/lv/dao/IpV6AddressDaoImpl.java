...
public class IpV6AddressDaoImpl implements IpV6AddressDao {
	...
	public IpV6Address findByAddress(String address) {
		...
		Query query = entityManager.createNamedQuery(
				IpV6Address.FIND_BY_ADDRESS).setParameter(1, address);
		return (IpV6Address) Dao.find(query);
	}
	...
	public List<IpV6Address> getBlackList() {
		...
		Query query = entityManager.createNamedQuery(IpV6Address.GET_WHITELIST)
	...
	@SuppressWarnings("unchecked")
	private List<IpV6Address> getColorList(int from, int count,
			boolean whiteList) {
		Query query = entityManager.createNamedQuery(IpV6Address.GET_WHITELIST)
				.setParameter(1, whiteList).setFirstResult(from)
				.setMaxResults(count);
		return (List<IpV6Address>) Dao.getRange(from, count, query);
	}
