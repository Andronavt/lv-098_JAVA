...
public class CountryDaoImpl extends DaoAbstract implements CountryDao {
	...
	public List<String> findCountryCodeListByStatus(boolean status,
			Class<? extends IpAddress> ipType) throws DBException {
		Query query = entityManager
				.createNamedQuery(Country.FIND_COUNTRY_CODE_LIST_BY_STATUS);
		query = query.setParameter(1, status);
		return query.getResultList();
	}

	@Override
	public String findCountryCodeByCountryName(String country,
			Class<? extends IpAddress> ipType) throws DBException {
		Query query = entityManager
				.createNamedQuery(Country.FIND_COUNTRY_CODE_BY_COUNTRY_NAME);
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<String> findCountryNameListByStatus(boolean status,
			Class<? extends IpAddress> ipType) throws DBException {
		Query query = entityManager
				.createNamedQuery(Country.FIND_COUNTRY_NAME_LIST_BY_STATUS);
		query = query.setParameter(1, status);
		return query.getResultList();
	}
